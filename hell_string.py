...
def main():
    ...
    print("Hello!! Welcome to KiaraBot Session Generator\n")
...
def generate_kiarabot_session():
    print("!!! KIARABOT SESSION !!!")
    print("One session for all KiaraBot's Project.")
    api_id = int(input("\nEnter APP ID here: "))
    api_hash = input("\nEnter API_HASH here: ")
    with PClient(name="kiaraluser", api_id=api_id, api_hash=api_hash, in_memory=True) as kiara:
        print("\nYour KIARABOT SESSION is saved in your telegram saved messages.")
        _session = kiara.export_session_string()
        kiara_session = kiarabot_session(_session)
        kiara.send_message(
            "me",
            f"#KIARABOT_SESSION \n\n`{kiara_session}`",
        )
...
def generate_pyro_session():
    ...
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with PClient(name="kiarauser", api_id=APP_ID, api_hash=API_HASH, in_memory=True) as kiarabot:
        print("\nYour KiaraBot Session Is sent in your Telegram Saved Messages.")
        kiarabot.send_message(
            "me",
            f"#KIARABOT #PYROGRAM\n\n`{kiarabot.export_session_string()}`",
        )


def generate_telethon_session():
    print("\nTelethon Session For KiaraBot!")
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with TelegramClient(StringSession(), APP_ID, API_HASH) as kiarabot:
        print("\nYour KiaraBot Session Is sent in your Telegram Saved Messages.")
        kiarabot.send_message(
            "me",
            f"#KIARALBOT #TELETHON \n\n`{kiarabot.session.save()}`",
        )


def generate_insta_session():
    print("Instagram Session For KiaraBot!")
    cl = IClient()
...
def kiarabot_session(session):
    ...
    error_msg = "Error in generating session! Report it in Kiara Chats"
    ...
    if len(session) in pyro_format.keys():
        ...
        # https://github.com/KiaralBoy-OP/Telethon/blob/v1/telethon/sessions/string.py
        ...
        return f"=={kiarabot('kiara')}{new_session}{kiarabot('bot')}=="
    else:
        return error_msg
