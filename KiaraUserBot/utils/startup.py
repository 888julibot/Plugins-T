...
from KiaraUserBot.helpers.int_str import make_int
from KiaraUserBot.version import __telever__


# Creates the logger group on first deploy and adds the helper bot
async def logger_id(client):
    desc = "A Bot Logger Group For Kiarabot. DO NOT LEAVE THIS GROUP!!"
    try:
        grp = await client(
            functions.channels.CreateChannelRequest(
                title="Kiarabot Logger", about=desc, megagroup=True
            )
        )
...
<b><i>»» <u><a href='https://t.me/Kiara_X_Upadates'>†hê Ừʂɛɤẞø†</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("KiaraBot Network", "https://t.me/Kiara_X_Upadates")]],
    )


# Joins the kiarabot chat and channel from all clients
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("@Kiara_X_Upadates"))
