...
from KiaraUserBot.clients.session import H2, H3, H4, H5, Kiara
from KiaraUserBot.utils.globals import CMD_LIST, LOAD_PLUG, SUDO_LIST
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                kiarareg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                kiarareg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(kiarareg + pattern)
    ...
    # blacklisted chats.
    # kiarabot will not respond in these chats.
...
def on(**args):
    def decorator(func):
        ...
        Kiara.add_event_handler(wrapper, events.NewMessage(**args))
        if H2:
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            Kiara.add_event_handler(func, events.MessageEdited(**args))
