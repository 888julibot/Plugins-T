...
from KiaraUserBot.helpers.pasters import pasty
...
# delete timeout
async def delete_kiara(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 10
    if event.sender_id in Config.SUDO_USERS:
        ...
        kiaraevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await kiaraevent.delete()


async def parse_error(event, error, auto_parse=True, delete=True, time=10):
    if delete:
        if auto_parse:
            await delete_kiara(event, f"**ERROR !!** \n\n`{error}`", time)
        else:
            await delete_kiara(event, f"**ERROR !!** \n\n{error}", time)
    else:
        if auto_parse:
