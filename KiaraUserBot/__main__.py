...
from KiaraUserBot.utils.plug import load_module, plug_channel
from KiaraUserBot.utils.startup import (join_it, logger_check, start_msg,
                                        update_sudo)
from KiaraUserBot.version import __kiaraver__

# Global Variables #
KIARA_PIC = "https://te.legra.ph/file/cb0bd62632a3a2b6b2726.jpg"
    ...
    for name in files:
        with open(name) as kiara:
            ...
            if shortname.replace(".py", "") in Config.UNLOAD:
                os.remove(Path(f"KiaraUserBot/plugins/{shortname}.py"))
            else:
                load_module(shortname.replace(".py", ""))


# Final checks after startup
async def kiara(total):
    await update_sudo()
...
async def start_kiarabot():
    try:
        ...
        await plug_load("KiaraUserBot/plugins/*.py")
