...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            kiararesponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            kiarafile = await event.client.download_media(kiararesponse, "./temp")
            return await unzip(kiarafile)
