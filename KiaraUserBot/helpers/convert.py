...
from KiaraUserBot.helpers.progress import progress
from KiaraUserBot.helpers.runner import runcmd
from KiaraUserBot.helpers.vids import take_screen_shot
from KiaraUserBot.utils.extras import edit_or_reply as eor
...
async def convert_to_image(event, client):
    kiara = await event.get_reply_message()
    if not (
        kiara.gif
        or kiara.audio
        or kiara.voice
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if kiara and kiara.photo:
        kiara_final = downloaded_file_name
    elif kiara.sticker and kiara.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        kiara_final = image_name20
    elif kiara.sticker and kiara.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        kiara_final = imgpath
    elif kiara.gif or kiara.video or kiara.video_note:
...
async def tgs_to_gif(file, tgs=False, video=False):
    if tgs:
        cmd = f"lottie_convert.py '{file}' 'kiarabot.gif'"
    elif video:
        cmd = f"ffmpeg -i '{file}' -c copy 'kiarabot.gif'"
    await runcmd(cmd)
    return 'kiarabot.gif'
