...
async def songs(event):
    ...
    if not query:
        return await parse_error(event, "Nothing given to search.")
    kiara = await eor(event, f"<b><i>Searching “ {query} ”</i></b>", parse_mode="HTML")
    try:
        song_dict = await song_search(kiara, query, 1)
        ...
        return await parse_error(kiara, f"__No song found. Maybe give different name or check spelling.__ \n`{str(e)}`", False)
    try:
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, kiara, c_time, upload_text)
            ),
            attributes=[
...
@kiara_cmd(pattern="lyrics(?:\s|$)([\s\S]*)")
async def lyrics(event):
    ...
    if results:
        ...
        if len(final) >= 4095:
            ...
            await kiara.edit(f"**Lyrics too big! Get it from here:** \n\n• [{title}]({link})", link_preview=False)
        else:
            await kiara.edit(final, parse_mode="HTML")
    else:
        await parse_error(kiara, "Unexpected Error Occured.")


@kiara_cmd(pattern="wsong(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
        return await eor(event, "Reply to a mp3 file.")
    rply = await event.get_reply_message()
    chat = "@auddbot"
    kiara = await eor(event, "Trying to identify song...")
