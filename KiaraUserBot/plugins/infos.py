...
<b>⚡ <a href='https://t.me/KiaraBot_Network'>From DataBase of KiaraBot</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@kiara_cmd(pattern="chatinfo(?:\s|$)([\s\S]*)")
async def info(event):
    kiara = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await kiara.edit(caption, parse_mode="HTML")
    except Exception as e:
        print("Exception:", e)
        await parse_error(kiara, "An unexpected error has occurred.")
    return
...
@kiara_cmd(pattern="admins(?:\s|$)([\s\S]*)")
async def _(event):
...
@kiara_cmd(pattern="id$")
async def _(event):
    kiara = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await kiara.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
