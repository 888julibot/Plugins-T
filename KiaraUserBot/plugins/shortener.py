...
@kiara_cmd(pattern="shorten(?:\s|$)([\s\S]*)")
async def shortener(event):
    ...
    if not query:
        return await parse_error(event, "Nothing given to short.")
    kiara = await eor(event, f"<b><i>Shortening “ {query} ”</i></b>", parse_mode="HTML")
    link = short(Config.SHORTENER_API, query)
    if not link:
        return await parse_error(kiara, f"__There was an error while shortening the link.__ \n`Check logs for more details.`", False)
    await kiara.edit(f"**••• Shortened Link •••** \n\n__» Short Url:__ {link} \n» Original Url: {query}")
