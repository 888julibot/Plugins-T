...
async def _(event):
    ...
    if len(lists) == 3:
        ...
        if stripped.startswith("-s"):
            ...
            url = stripped
    _, _, kiara_memtion = await client_id(event)
    reply = await event.get_reply_message()
    if reply:
        kiara = await eor(event, "`Analysing...`")
        file_name = "rmbg.png"
        try:
            ...
            await kiara.edit("`Removing background of this media`")
            file_name = toimage(file_name, "temp.jpeg")
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif url:
        kiara = await eor(event, "`Removing Background of this media`")
    ...
    remove_bg_image = "KiaraBot.png"
    if "image" in contentType:
        with open("KiaraBot.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        return await parse_error(kiara, response.content.decode('UTF-8'))
    if flag and flag == "-s":
        file = tosticker(remove_bg_image, filename="KiaraBot.webp")
        ...
        await event.client.send_file(
            event.chat_id,
            file,
            caption=f"**Background removed by** {kiara_memtion}",
            force_document=True,
        )
    else:
