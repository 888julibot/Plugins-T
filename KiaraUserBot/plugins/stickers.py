...
from KiaraUserBot.DB.gvar_sql import addgvar, gvarstat
from KiaraUserBot.plugins import *


@kiara_cmd(pattern="kang(?:\s|$)([\s\S]*)")
async def kang(event):
    kiara = await eor(event, "__**Starting sticker kang process ...**__")
    ...
    ForGo10God, kiara_USER, _ = await client_id(event)
    nick = f"@{user.username}" if user.username else kiara_USER
    ...
    if reply and reply.media:
        if reply.photo:
            ...
            if reply.media.document.mime_type == "video/webm":
                await kiara.edit("__Oow! A video sticker ...__ **[ ENCODING ]**")
                ...
                await kiara.edit("__Oow! A video ...__ **[ CONVERTING ]**")
                await VSticker(event, reply)
            is_vid = True
            photo = 1
        else:
            return await eod(kiara, "__Can't kang that 🔪__")
    else:
        return await eod(kiara, "__Can't kang that 🔪__")
    await kiara.edit("__**Adding this sticker to your pack...**__")
    ...
    if (
        ...
        async with event.client.conversation("@Stickers") as conv:
            try:
                ...
                while ("50" in resp.message) or ("120" in resp.message) or is_vid:
                    ...
                    if resp.message == "Invalid set selected.":
                        ...
                        if "Sorry, the file type is invalid." in rsp.text:
                            return await eod(kiara, "`Failed to add sticker, use` @Stickers `bot to add the sticker manually.`")
                ...
                return await parse_error(kiara, "__Unblock @Stickers and try again.__", False)
    else:
        await kiara.edit("__**Preparing a new pack....**__")
        async with event.client.conversation("@Stickers") as conv:
            await conv.send_message(cmd)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
    ...
    await tbot.send_message(
        Config.LOGGER_ID,
        f"#KANG #STICKER \n\nA sticker has been kanged into the pack of {kiara_USER}. Click below to see the pack!",
        buttons=[
          [Button.url("View Pack", f"t.me/addstickers/{packname}")],
          [Button.url(kiara_USER, f"tg://user?id={ForGo10God}")],
        ],
        parse_mode=None,
    )
    await eod(
...
@kiara_cmd(pattern="pkang(?:\s|$)([\s\S]*)")
async def _(event):
    kiara = await eor(event, "`Preparing pack kang...`")
    reply = await event.get_reply_message()
    ForGo10God, kiara_USER, _ = await client_id(event)
    ...
    user = await event.client.get_me()
    un = f"@{user.username}" if user.username else kiara_USER
    ...
    if reply and reply.media and reply.media.document.mime_type == "image/webp":
        kiara_id = reply.media.document.attributes[1].stickerset.id
        kiara_hash = reply.media.document.attributes[1].stickerset.access_hash
        got_stcr = await event.client(
            functions.messages.GetStickerSetRequest(
                stickerset=types.InputStickerSetID(id=kiara_id, access_hash=kiara_hash)
            )
        )
        stcrs = []
        ...
        if x is None:
            ...
            pack = int(x) + 1
        await kiara.edit("`Starting kang process...`")
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ForGo10God,
                    title=pname,
                    short_name=f"kiara_{un_}_V{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
            addgvar("PKANG", pack)
            ...
            await kiara.edit("`Pack name already occupied... making new pack`")
            pack = int(pack) + 1
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=ForGo10God,
        ...
        await tbot.send_message(
            Config.LOGGER_ID,
            f"#PKANG #STICKER \n\nA sticker pack has been kanged by {kiara_USER}. Click below to see the pack!",
            buttons=[
        ...
        await kiara.edit("Unsupported File. Please Reply to a sticker only.")
    await to_del.delete()


@kiara_cmd(pattern="stkrinfo$")
async def get_pack_info(event):
    ...
    try:
        ...
        kiara = await eor(event, "`Fetching details of the sticker pack, please wait..`")
    ...
    await kiara.edit(OUTPUT, parse_mode='HTML')


@kiara_cmd(pattern="delst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
        return await eod(event, "Reply to a sticker to delete it.")
    kiara = await eor(event, "🥴 `Deleting sticker...`")
    async with event.client.conversation("@Stickers") as conv:
        try:
            ...
            return await parse_error(kiara, "__Unblock @Stickers and try again.__", False)
        await kiara.edit(fourth.text)


@kiara_cmd(pattern="editst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
    ...
    emoji = lists[1].strip()
    kiara = await eor(event, f"📝 `Editing sticker emoji to {emoji}`")
    async with event.client.conversation("@Stickers") as conv:
        try:
            ...
            if fourth.text.startswith("Current emoji:"):
                ...
                return await eod(kiara, "That's not your sticker!")
        except YouBlockedUserError:
            return await parse_error(kiara, "__Unblock @Stickers and try again.__", False)
...
async def sticklet(event):
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@KiaraFonts")
    ...
    image_stream.name = "Kiarabot.webp"
