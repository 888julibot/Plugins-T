...
@kiara_cmd(pattern="carbon(?:\s|$)([\s\S]*)")
async def carbon(event):
    kiara = await eor(event, "__Making carbon ... 25%__")
    ...
    if reply:
        ...
        if len(lists) < 2:
            return await parse_error(kiara, "Nothing given to make carbon.")
        _text = lists[1]
    text = deEmojify(_text)
    code = quote_plus(text)

    await kiara.edit("__Making carbon ... 50%__")
    ...
    await kiara.edit("__Making carbon ... 75%__")
    ...
    await kiara.edit("__Making carbon ... 100%__")
    await event.client.send_file(
        event.chat_id,
        "./carbon.png",
        caption=f"**Carbonized by:** {kiara_mention}",
        force_document=True,
        reply_to=reply,
    )
