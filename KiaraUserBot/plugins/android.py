...
@kiara_cmd(pattern="app(?:\s|$)([\s\S]*)")
async def apk(event):
    lists = event.text.split(" ", 1)
    if not len(lists) == 2:
        return await parse_error(event, "Invalid syntax.")
    app_name = lists[1].strip()
    kiara = await eor(event, f"__Searching for__ `{app_name}` __...__")
    try:
        ...
        await kiara.edit(app_details, link_preview=True, parse_mode="HTML")
    except IndexError:
        await parse_error(kiara, "No result found in search. Please enter **Valid app name**", False)
    except Exception as err:
        await parse_error(kiara, err)


@kiara_cmd(pattern="magisk$")
async def _(magisk):
    if magisk.fwd_from:
        return
...
@kiara_cmd(pattern="device(?:\s|$)([\s\S]*)")
...
@kiara_cmd(pattern="codename(?: |)([\S]*)(?: |)([\s\S]*)")
