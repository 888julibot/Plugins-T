...
async def all_groups_id(kiara):
    kiaragroups = []
    async for dialog in kiara.client.iter_dialogs():
        entity = dialog.entity
        if isinstance(entity, Channel) and entity.megagroup:
            kiaragroups.append(entity.id)
    return kiaragroups


@kiara_cmd(pattern="frwd$")
async def _(event):
