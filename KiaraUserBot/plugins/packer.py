...
@kiara_cmd(pattern="unpack$")
async def _(event):
    kiara = await eor(event, "**Unpacking...**")
...
@kiara_cmd(pattern="pack(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[6:]
    kiara = await eor(event, f"Packing into `{input_str}`")
    a = await event.get_reply_message()
    b = open(input_str, "w")
    b.write(str(a.message))
    b.close()
    await kiara.edit(f"Uploading `{input_str}`")
