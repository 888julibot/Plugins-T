...
@kiara_cmd(pattern="feye$")
async def fun(event):
    path = "omk"
    kiara = await eor(event, "Editing In Progress...")
...
@kiara_cmd(pattern="warp$")
async def fun(event):
    path = "omk"
    kiara = await eor(event, "Warping In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
...
@kiara_cmd(pattern="distort$")
async def fun(event):
    path = "omk"
    kiara = await eor(event, "Distortion In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
    file_name = "dist.jpg"
    hehe = path + "/" + file_name
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            return await parse_error(kiara, "__Unblock @image_deepfrybot and try again.__", False)
