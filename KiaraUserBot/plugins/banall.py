...
@kiara_cmd(pattern="kickall$", allow_sudo=False)
async def _(event):
    ...
    kiara = await eor(event, "**Bleck Magik Started...**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
            await asyncio.sleep(0.5)
    await kiara.edit("**Bleck Magik Done...**")
...
@kiara_cmd(pattern="banall$", allow_sudo=False)
async def _(event):
    ForGo10God, _, _ = await client_id(event)
    result = await event.client(
        functions.channels.GetParticipantRequest(event.chat_id, ForGo10God)
    ...
    if not result.participant.admin_rights.ban_users:
        return await parse_error(event, "Need ban rights to do this.")
    kiara = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
            await asyncio.sleep(0.5)
    await kiara.edit("**Bleck Magik Completed...**")
    await event.client.send_message(
        Config.LOGGER_ID,
...
async def _(event):
    ...
    q = 0
    r = 0
    kiara = await eor(event, "Searching Participant Lists.")
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await parse_error(kiara, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await kiara.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
...
@kiara_cmd(pattern="blockall(?:\s|$)([\s\S]*)")
async def block_all(event):
    if event.is_private:
        return
    ForGo10God, _, _ = await client_id(event)
    kiara = await eor(event, "__Starting to block all members in this group ...__")
    failed = 0
    success = 0
    await kiara.edit("**MASS-BLOCK in action !!**")
    async for user in event.client.iter_participants(event.chat_id):
        try:
            ...
            LOGS.info(str(e))
            failed += 1
    await kiara.edit(f"**MASS-BLOCK completed !!** \n\n__Blocked:__ `{success} users` \n__Failed:__ `{failed} users`")


@kiara_cmd(pattern="blockc(?:\s|$)([\s\S]*)")
async def block_contacts(event):
    if event.is_private:
        return
    ForGo10God, _, _ = await client_id(event)
    kiara = await eor(event, "__Starting to block my contacts in this group ...__")
    failed = 0
    success = 0
    await kiara.edit("**MASS-BLOCK in action !!**")
    async for user in event.client.iter_participants(event.chat_id):
        ...
        if user in result_ompho:
            ...
            return await eod(kiara, "No cantact found in this group.")
    await kiara.edit(f"**MASS-BLOCK completed !!** \n\n__Blocked:__ `{success} users` \n__Failed:__ `{failed} users`")


@kiara_cmd(pattern="blocknc(?:\s|$)([\s\S]*)")
async def block_noncontacts(event):
    if event.is_private:
        return
    ForGo10God, _, _ = await client_id(event)
    kiara = await eor(event, "__Starting to block non contacts in this group ...__")
    failed = 0
    success = 0
    await kiara.edit("**MASS-BLOCK in action !!**")
    async for user in event.client.iter_participants(event.chat_id):
        ...
        if user not in result_ompho:
            ...
            return await eod(kiara, "No non-cantact found in this group.")
    await kiara.edit(f"**MASS-BLOCK completed !!** \n\n__Blocked:__ `{success} users` \n__Failed:__ `{failed} users`")


@kiara_cmd(pattern = "unblockall(?:\s|$)([\s\S]*)")
async def ublock_all(event):
    if event.is_private:
        return
    ForGo10God, _, _ = await client_id(event)
    kiara = await eor(event, "__Starting to unblock all users in this group ...__")
    failed = 0
    success = 0
    await kiara.edit("**MASS-UNBLOCK in action !!**")
    async for user in event.client.iter_participants(event.chat_id):
        try:
            ...
            LOGS.info(str(e))
            failed += 1
    await kiara.edit(f"**MASS-UNBLOCK completed !!** \n\n__Unblocked:__ `{success} users` \n__Failed:__ `{failed} users`")    
