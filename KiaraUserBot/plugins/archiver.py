...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await eod(kiara, downloaded_file_name)
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            directory_name = downloaded_file_name
            await kiara.edit("Finished downloading to my local")
            ...
            try:
                ...
                pass
            await eod(kiara, "Task Completed")
        except Exception as e:
            await parse_error(kiara, e)
...
@kiara_cmd(pattern="unzip$")
async def _(event):
    kiara = await eor(event, "Processing ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if not event.reply_to_msg_id:
        return await parse_error(kiara, "Reply to zip file!")
    else:
        ...
        try:
            ...
            await kiara.edit("Stored the zip to `{}` in {} seconds.".format(downloaded_file_name, ms))
        extracted = "./extracted"
        with zipfile.ZipFile(downloaded_file_name, "r") as zip_ref:
            zip_ref.extractall(extracted)
        filename = sorted(get_lst_of_files(extracted, []))
        await kiara.edit("Unzipping now")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            ms = (end - start).seconds
            await kiara.edit(f"Stored the tar to `{downloaded_file_name}` in {ms} seconds.")
        ...
        filename = sorted(get_lst_of_files(extracted, []))
        await kiara.edit("Untarring now")
        for single_file in filename:
            if os.path.exists(single_file):
                caption_rts = os.path.basename(single_file)
