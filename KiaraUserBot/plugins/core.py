...
async def kk(event):
    ...
    ForGo10God, kiara_USER, kiara_mention = cids[0], cids[1], cids[2]
    cmd = "ls KiaraUserBot/plugins"
...
<b><a href='https://t.me/Kiara_X_Upadates'>@Kiara_X_Upadates</a></b>
"""
...
async def send(event):
    ...
    the_plugin_file = "./KiaraUserBot/plugins/{}.py".format(input_str.lower())
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "./KiaraUserBot/plugins/",  # pylint:disable=E0602
                )
            )
            if owo != "-f":
...
async def uninstall(event):
    ...
    dir_path = f"./KiaraUserBot/plugins/{shortname}.py"
    try:
...
CmdHelp("core").add_command(
    "install", "<reply to a .py file>", "Installs the replied python file if suitable to Ừʂɛɤẞø†'s codes.`\n**🚩 Flags :** `-f"
).add_command(
    "uninstall", "<plugin name>", "Uninstalls the given plugin from Ừʂɛɤẞø†. To get that again do .restart", "uninstall alive"
).add_command(
