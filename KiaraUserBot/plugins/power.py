...
import heroku3
from KiaraUserBot.DB.gvar_sql import addgvar, delgvar, gvarstat
...
@kiara_cmd(pattern="restart$")
async def re(kiara):
    event = await eor(kiara, "Restarting Ừʂɛɤẞø† ...")
    try:
        await restart(event)
...
@kiara_cmd(pattern="reload$")
async def rel(event):
    await eor(event, "**Reloaded KiaraBot!** \n\n__This might take a minute.__")
    await reload_kiarabot()


@kiara_cmd(pattern="shutdown$")
async def down(event):
    await eor(event, "**[ ⚠️ ]** \n**Ừʂɛɤẞø† is now turned off. Manually turn it on to start again.**")
...
async def sett(event):
    ...
    val = lists[2].strip()
    kiara = await eor(event, f"**Setting variable** `{var}` **as** `{val}`")
    if var == "":
        return await parse_error(kiara, f"__Invalid Syntax !!__ \n__Try:__ `{hl}svar VARIABLE_NAME variable_value`", False)
    elif val == "":
        return await parse_error(kiara, f"__Invalid Syntax !!__ \n__Try:__ `{hl}svar VARIABLE_NAME variable_value`", False)
    if var not in db_config:
        return await parse_error(kiara, f"__No DB Variable:__ `{var}`. \n__Check spelling or get full list by__ `{hl}vars -db`", False)
    try:
        addgvar(var, val)
    except Exception as e:
        return await parse_error(kiara, e)
    await eod(kiara, f"**Variable Added Successfully!!** \n\n**• Variable:** `{var}` \n**» Value:** `{val}`")


@kiara_cmd(pattern="gvar(?:\s|$)([\s\S]*)")
async def gett(event):
    lists = event.text.split(" ", 2)
    if len(lists) < 2:
        return await parse_error(event, f"__Invalid Syntax !!__ \n__Try:__ `{hl}gvar VARIABLE_NAME`", False)
    var = lists[1].strip().upper()
    kiara = await eor(event, f"**Getting variable** `{var}`")
    if var == "":
        return await parse_error(kiara, f"__Invalid Syntax !!__ \n__Try:__ `{hl}gvar VARIABLE_NAME`", False)
    if var not in db_config:
        return await parse_error(kiara, f"__No DB Variable:__ `{var}`. \n__Check spelling or get full list by__ `{hl}vars -db`", False)
    try:
        ...
        return await parse_error(kiara, e)
    await kiara.edit(f"**• OS VARIABLE:** `{var}`\n**» OS VALUE :** `{os_v}`\n\n**• DB VARIABLE:** `{var}`\n**» DB VALUE :** `{db_v}`\n")


@kiara_cmd(pattern="dvar(?:\s|$)([\s\S]*)")
async def dell(event):
    lists = event.text.split(" ", 2)
    if len(lists) < 2:
        return await parse_error(event, f"__Invalid Syntax !!__ \n__Try:__ `{hl}dvar VARIABLE_NAME`", False)
    var = lists[1].strip().upper()
    kiara = await eor(event, f"**Deleting Variable** `{var}`")
    if var == "":
        return await parse_error(kiara, f"__Invalid Syntax !!__ \n__Try:__ `{hl}dvar VARIABLE_NAME`", False)
    if var not in db_config:
        return await parse_error(kiara, f"__No DB Variable:__ `{var}`. \n__Check spelling or get full list by__ `{hl}vars -db`", False)
    if gvarstat(var):
        try:
            x = gvarstat(var)
            delgvar(var)
            await eod(kiara, f"**Deleted Variable Successfully!!** \n\n**• Variable:** `{var}` \n**» Value:** `{x}`")
        except Exception as e:
            await parse_error(kiara, e)
    else:
        await parse_error(kiara, f"`{var}` __does not exists.__", False)
