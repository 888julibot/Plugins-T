...
@kiara_cmd(pattern="lock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[6:]
    cid = await client_id(event)
    kiara_mention = cid[2]
    kiara = await eor(event, f"Trying to lock `{text}`")
    if text == "":
        return await parse_error(kiara, "Nothing given to lock in chat.")
    locker = lock(text)
    if not locker:
        return await eod(
            kiara,
    ...
    await event.client.send_file(
        event.chat_id,
        restlo,
        caption=f"{kiara_mention} Locked `{text}` \n__Cause its Rest Time !!__",
    )
    await kiara.delete()


@kiara_cmd(pattern="unlock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[8:]
    cid = await client_id(event)
    kiara_mention = cid[2]
    kiara = await eor(event, f"Trying to unlock `{text}`")
    if text == "":
        return await parse_error(kiara, "Nothing given to unlock in chat.")
    ...
    await event.client.send_file(
        event.chat_id, shuru, caption=f"**{kiara_mention} unlocked** `{text}`"
    )
    await kiara.delete()


@kiara_cmd(pattern="ltype$")
async def _(event):
    await eor(
        event,
