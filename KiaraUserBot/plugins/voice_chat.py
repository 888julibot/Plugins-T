...
@kiara_cmd(pattern="vcinvite$")
async def _(event):
    kiara = await eor(event, "`🧐 Inviting Users To Voice Chat....`")
    users = []
    i = 0
    ...
    for k in lists:
        try:
            ...
            pass
    await parse_error(kiara, f"**🚀 Invited** `{i}` **users to Voice Chat.**")
