...
from KiaraUserBot.clients.client_list import get_user_id
...
async def pong(event):
    ...
    if PIC:
        await event.client.send_file(
            event.chat_id,
            file=PIC,
            caption=ping_txt.format(ms, uptime, kiara_mention),
            parse_mode="HTML",
        )
        await kiara.delete()
    else:
        await kiara.edit(ping_txt.format(ms, uptime, kiara_mention), parse_mode="HTML")


@kiara_cmd(pattern="limits$")
...
@kiara_cmd(pattern="kickme$")
async def leave(event):
    kiara = await eor(event, "😪 **KThnxBye** See u all in kiara!!")
    time.sleep(1)
    if "-" in str(event.chat_id):
        await event.client(LeaveChannelRequest(event.chat_id))
    else:
        await eod(kiara, "**Is this even a group?😑**")


@kiara_cmd(pattern="dc$")
async def _(event):
    result = await event.client(functions.help.GetNearestDcRequest())
    await eor(event, result.stringify())


@kiara_cmd(pattern="config$")
async def _(event):
...
@kiara_cmd(pattern="vars(?:\s|$)([\s\S]*)")
async def lst(event):
    flag = str(event.text[6:9]).lower()
    if flag and flag == "-db":
        kiara = await eor(event, "Getting DB variables..")
        dbx = "**• List of DB Variables:** \n\n"
        for data in db_config:
            dbx += f"» `{data}`\n"
        await kiara.edit(dbx)
    else:
        kiara = await eor(event, "Getting configs list...")
        osx = "**• List of OS Configs:** \n\n"
        for data in os_config:
            osx += f"» `{data}`\n"
        await kiara.edit(osx)


@kiara_cmd(pattern="schd(?:\s|$)([\s\S]*)")
async def _(event):
...
async def _(event):
    ...
    if reply and len(lists) >= 2:
        ...
        await eod(event, f"**SYNTAX EXAMPLE**\n\n~ `{hl}dm @ForGo10God Hey Kiara!` \n~ `{hl}dm @ForGo10God <reply to a msg>`")


@kiara_cmd(pattern="chain$")
async def _(event):
    kiara = await eor(event, "Counting...")
    count = -1
    ...
    while message:
        ...
        message = reply
        count += 1
    await kiara.edit(f"⛓️ **Chain length :**  `{count}`")
