...
async def _(event):
    ...
    kiara = await eor(event, "Making Telegraph Link....")
    ...
    if reply:
        ...
        await eod(kiara, "Reply to a message to get a permanent telegra.ph link.")


@kiara_cmd(pattern="tgraph(?:\s|$)([\s\S]*)")
async def _(event):
    kiara = await eor(event, "Making telegraph post ...")
    reply = await event.get_reply_message()
    lists = event.text.split(" ", 1)
    if not reply:
    ...
    title = None
    auth = "[ †he Ừʂɛɤẞø† ]"
    url = "https://t.me/Kiara_X_Upadates"
