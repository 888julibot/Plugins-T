...
@kiara_cmd(pattern="imdb(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    lists = event.text.split(" ", 1)
    if not len(lists) == 2:
        return await parse_error(event, "Nothing given to search.")
    kiara = await eor(event, f"Searching `{lists[1]}` in IMDb...")
    try:
        # IB: catuserbot.
...
<b>Director :</b> <code>{mov_director}</code>"""
        ...
        await parse_error(kiara, f"__No movie found with name__ `{lists[1]}`", False)
    except Exception as e:
        await parse_error(kiara, e)


@kiara_cmd(pattern="watch(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.pattern_match.group(1)
    kiara = await eor(event, "Finding Sites...")
    ...
    if thumb_link:
        ...
        open("kiarabot_watch.jpg", "wb").write(banner.content)
        title_img = "kiarabot_watch.jpg"
