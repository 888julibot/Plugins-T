...
@kiara_cmd(pattern="rename(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[8:]
    if input_str == "":
        return await eod(event, "Give a new file name.")
    kiara = await eor(event, f"Renaming to `{input_str}`")
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await event.client.send_file(
                ...
                thumb=thumb,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, kiara, c_time, "Uploading ...", downloaded_file_name)
                ),
            )
        ...
        await kiara.edit(f"**Syntax Wrong !!** \n\n• `{hl}rnupload new file name` as reply to a telegram file.")


@kiara_cmd(pattern="rnsupload(?:\s|$)([\s\S]*)")
async def _(event):
