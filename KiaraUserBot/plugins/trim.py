...
@kiara_cmd(pattern="vtrim(?:\s|$)([\s\S]*)")
async def video(event):
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    kiara = await eor(event, "Starting video trim ...")
    reply = await event.get_reply_message()
    if not reply:
        return await parse_error(kiara, "Reply to a video/gif to trim.")
    dl_start = datetime.datetime.now()
    media = media_type(reply)
    if media not in ["Video", "Round Video", "Gif"]:
        return await parse_error(kiara, "Only video/gif is supported.")
    await kiara.edit("Starting to download media ...")
    try:
        c_time = time.time()
        downloaded_file_name = await event.client.download_media(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, kiara, c_time, "Downloading ...")
            ),
        )
    ...
    if len(lists) == 3:
        ...
        if o is None:
            return await parse_error(kiara, f"Unexpected error occured.")
        try:
            await kiara.edit("**Trimmed!** Uploading now ...")
            c_time = time.time()
            await event.client.send_file(
                ...
                allow_cache=False,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, kiara, c_time, "**Trimmed!** Uploading now ...")
                ),
            )
        except Exception as e:
            return await parse_error(kiara, e)
        ...
        if o is None:
            return await parse_error(kiara, "Unexpected error occured.")
        try:
            await kiara.edit("**Screenshot Completed!** Uploading now ...")
            c_time = time.time()
            await event.client.send_file(
                event.chat_id,
                o,
                ...
                allow_cache=False,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, kiara, c_time, "**Screenshot Completed!** Uploading now ...")
                ),
            )
        except Exception as e:
            return await parse_error(kiara, e)
    else:
        return await parse_error(kiara, f"__Give proper command:__ \n__Example:__ `{hl}vtrim 60 70`", False)
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(kiara, f"**Trimmed!!**\n__Time taken:__ `{ms} seconds`")
    os.remove(downloaded_file_name)
    os.remove(o)


@kiara_cmd(pattern="atrim(?:\s|$)([\s\S]*)")
async def audio(event):
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    kiara = await eor(event, "Starting audio trim ...")
    reply = await event.get_reply_message()
    if not reply:
        return await parse_error(kiara, "Reply to a media file to trim.")
    dl_start = datetime.datetime.now()
    media = media_type(reply)
    if media not in ["Video", "Audio", "Voice", "Round Video", "Gif"]:
        return await parse_error(kiara, "Only media file is supported.")
    await kiara.edit("Starting to download media ...")
    try:
        c_time = time.time()
        downloaded_file_name = await event.client.download_media(
