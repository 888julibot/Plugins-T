from KiaraUserBot.DB.pmlogger_sql import (add_nolog, del_nolog, get_all_nolog,
                                          is_nolog)
from KiaraUserBot.plugins import *


@kiara_cmd(pattern="save(?:\s|$)([\s\S]*)")
async def _(event):
...
@kiara_handler(func=lambda e: e.is_private, incoming=True)
async def _(event):
    if Config.PM_LOGGER == 0:
        return
    ForGo10God, _, _ = await client_id(event)
