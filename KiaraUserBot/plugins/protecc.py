...
from KiaraUserBot.DB.husb_sql import (add_hus_grp, get_all_hus_grp, is_husb,
                                      rm_hus_grp)
from KiaraUserBot.DB.waifu_sql import add_grp, get_all_grp, is_harem, rm_grp
...
@kiara_cmd(pattern="pt(?:\s|$)([\s\S]*)")
async def _(event):
    BASE_URL = "http://images.google.com"
    if event.reply_to_msg_id:
        kiara = await eor(event, "Hmm..")
        ...
        await kiara.edit(f"/protecc {prs_text}", parse_mode="HTML", link_preview=False)
    else:
...
async def _(event):
    ...
    if is_harem(str(event.chat_id)):
        try:
            ...
            alls = div.find("a")
            text = alls.text
            kiara = await event.client.send_message(
                event.chat_id,
                f"/protecc {text}",
                reply_to=event,
            )
...
@kiara_cmd(pattern="adwaifu(?:\s|$)([\s\S]*)")
async def _(event):
...
@kiara_cmd(pattern="rmwaifu(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, kiara_mention = await client_id(event)
    if not event.is_group:
        return await parse_error(event, "Autowaifu works in groups only !!")
...
@kiara_cmd(pattern="rmhusb(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, kiara_mention = await client_id(event)
    if not event.is_group:
        return await parse_error(event, "Autohusbando works in groups only !!")
...
@kiara_cmd(pattern="autowaifu$")
async def _(event):
    kiara = await eor(event, "Fetching Autowaifu chats...")
    all_grp = get_all_grp()
...
@kiara_cmd(pattern="autohusbando$")
async def _(event):
    kiara = await eor(event, "Fetching Autohusbando chats...")
