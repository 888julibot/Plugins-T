...
@kiara_cmd(pattern="commit(?:\s|$)([\s\S]*)")
async def download(event):
    ...
    kiarabot = await eor(event, "Processing ...")
    ...
    try:
        ...
        await kiarabot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, path, branch, kiarabot)


async def git_commit(file_name, path, branch, kiarabot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await kiarabot.edit("`File Already Exists`")
            create_file = False
    path = path
    file_name = file_name
    if create_file == True:
        ...
        try:
            repo.create_file(
                path,
                f"Uploaded file {file_name} by Ừʂɛɤẞø†",
                commit_data,
                branch=branch,
            )
            ...
            await parse_error(kiarabot, "Cannot Upload File")
    else:
        return await parse_error(kiarabot, "Committed Suicide")
...
@kiara_cmd(pattern="github(?:\s|$)([\s\S]*)")
async def _(event):
...
CmdHelp("github").add_command(
    "commit", "<reply to a file> <path>|<branch>", "Uploads the file on github repo as provided in Heroku Config GIT_REPO_NAME. In short makes a commit to git repo from Userbot", "commit ./kiarabot/plugins/example.py|master"
).add_command(
