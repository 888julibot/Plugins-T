...
from KiaraUserBot.DB.mute_sql import is_muted, mute, unmute
...
@kiara_cmd(pattern="promote(?:\s|$)([\s\S]*)")
@errors_handler
async def promote(event):
    ...
    kiaraevent = await eor(event, "`Promoting User...`")
...
@kiara_cmd(pattern="demote(?:\s|$)([\s\S]*)")
@errors_handler
async def demote(event):
    ...
    if not admin and not creator:
        return await parse_error(event, NO_ADMIN)
    kiaraevent = await eor(event, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        return await parse_error(kiaraevent, NO_PERM)
    await kiaraevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{event.chat.title}`")
    await event.client.send_message(
        Config.LOGGER_ID,
        "#DEMOTE\n"
        f"\n**USER:** [{user.first_name}](tg://user?id={user.id})"
...
@kiara_cmd(pattern="mute(?:\s|$)([\s\S]*)")
async def muth(event):
    x = await client_id(event)
    Kiara_X_Assistant = x[0]
    if event.is_private:
        kiara = await eor(event, "**Enough of your bullshit  !!**")
        await event.get_reply_message()
        if is_muted(event.chat_id, event.chat_id):
            return await eod(kiara, "User is already muted here 🥴")
        if event.chat_id == Kiara_X_Assistant:
            return await parse_error(kiara, "You can't mute yourself !")
        try:
            mute(event.chat_id, event.chat_id)
            await eod(kiara, "**Muted this user !**")
        except Exception as e:
            return await parse_error(kiara, e)
    elif event.is_group:
        kiara = await eor(event, "`Muting...`")
        ...
        if event.reply_to_msg_id:
            ...
            if input_str.isdigit():
                try:
                    ...
                    return await parse_error(kiara, ve)
            else:
                userid = (await event.client.get_entity(input_str)).id
                name = (await event.client.get_entity(userid)).first_name
        else:
            return await parse_error(kiara, "I need an user to mute !!")
        if userid == Kiara_X_Assistant:
            return await parse_error(kiara, "You can't mute yourself !")
        if str(userid) in DEVLIST:
            return await parse_error(kiara, "Cant mute my developers !")
        if Kiara_X_Assistant not in admin_:
            return await parse_error(kiara, NO_PERM)
        if userid in admin_:
            if is_muted(userid, event.chat_id):
                return await eod(kiara, "Admin already muted ♪～(´ε｀ )")
            try:
...
@kiara_cmd(pattern="unmute(?:\s|$)([\s\S]*)")
async def nomuth(event):
    x = await client_id(event)
    Kiara_X_Assistant = x[0]
    kiara = await eor(event, "`Unmuting ...`")
    if event.is_private:
        if not is_muted(event.chat_id, event.chat_id):
            return await eod(kiara, "Not even muted !!")
        try:
            unmute(event.chat_id, event.chat_id)
            await eod(kiara, "User unmuted successfully !")
        except Exception as e:
            await parse_error(kiara, e)
    elif event.is_group:
        input_str = event.pattern_match.group(1)
        ...
        if event.reply_to_msg_id:
            ...
            if input_str.isdigit():
                try:
                    ...
                    return await parse_error(hkiara, ve)
            else:
                userid = (await event.client.get_entity(input_str)).id
                name = (await event.client.get_entity(userid)).first_name
        else:
            return await parse_error(kiara, "I need a user to unmute!!")
        if Kiara_X_Assistant not in admin_:
            return await parse_error(kiara, NO_PERM)
        if userid in admin_:
...
@kiara_cmd(pattern="ban(?:\s|$)([\s\S]*)")
@errors_handler
async def ban(event):
    kiaraevent = await eor(event, "`Banning Nigga...`")
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
    ...
    if not user:
        return await parse_error(kiaraevent, "`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await eod(kiaraevent, "**Say again? Ban my creator??**")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        await parse_error(kiaraevent, NO_PERM)
        return
    try:
        reply = await event.get_reply_message()
        if reply:
...
async def nothanos(event):
    ...
    if not admin and not creator:
        await parse_error(event, NO_ADMIN)
        return
    kiaraevent = await eor(event, "`Unbanning...`")
    user, _ = await get_user_from_event(event)
    if not user:
        return
    try:
        ...
        await parse_error(kiaraevent, "Invalid UserId!! Please Recheck it!!")


@kiara_cmd(pattern="pin(?:\s|$)([\s\S]*)")
@errors_handler
async def pin(event):
    chat = await event.get_chat()
...
async def kick(event):
    ...
    kiaraevent = await eor(event, "`Kicking...`")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await parse_error(kiaraevent, e)
        return
    if reason:
        await kiaraevent.edit(
...
@kiara_cmd(pattern="zombies(?:\s|$)([\s\S]*)")
async def rm_deletedacc(event):
    ...
    if action == "clean":
        ...
        if not admin and not creator:
            return await parse_error(event, NO_ADMIN)
        kiara = await eor(event, "__🧹 Purging Zombies from here ...__")
        del_u = 0
        del_a = 0
...
#                     await sleep(0.5)
                    ...
                    return await parse_error(kiara, "I don't have ban rights in this group")
                except UserAdminInvalidError:
                    del_a += 1
                except FloodWaitError as fw:
                    await kiara.edit(f"**FlooadWait:**\n\n__Sleeping for {fw.seconds} seconds__")
                    await sleep(fw.seconds)
        if del_u > 0:
            del_status = f"**Zombies Purged!!**\n\n**Cleaned:** `{del_u}`"
        if del_a > 0:
            del_status = f"**Zombies Purged!!**\n\n**Cleaned:** `{del_u}`\n\n`{del_a}` **Zombies Holds Immunity!!**"
        await kiara.edit(del_status)
        await event.client.send_message(
            Config.LOGGER_ID,
            f"#ZOMBIES\
            \n{del_status}\
        ...
        kiara = await eor(event, "**Searching For Zombies...**")
        async for user in event.client.iter_participants(event.chat_id):
            if user.deleted:
                del_u += 1
                await sleep(0.5)
