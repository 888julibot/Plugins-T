...
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        ...
        try:
            ...
            result = _out_
        kiarauser = await event.client.get_me()
        if kiarauser.username:
            curruser = kiarauser.username
        else:
            curruser = "@Kiara_X_Upadates"
        uid = os.geteuid()
        if uid == 0:
...
@kiara_cmd(pattern="eval(?:\s|$)([\s\S]*)")
async def _(event):
    if gvarstat("USE_EVAL") == "TRUE":
        cmd = "".join(event.text.split(maxsplit=1)[1:])
        if not cmd:
            return await eod(event, "`What should i run ?..`")
        kiaraevent = await eor(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
    ...
    "exec", "<command>", "Execute a Terminal command on KiaraBot server and shows details"
).add_command(
