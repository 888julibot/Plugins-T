...
async def _(event):
    ...
    OUTPUT = f"**Files in Ừʂɛɤẞø† DOWNLOADS Folder:**\n"
...
@kiara_cmd(pattern="rnsaved(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@kiara_cmd(pattern="rnlocal(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@kiara_cmd(pattern="delsave(?:\s|$)([\s\S]*)")
...
@kiara_cmd(pattern="delocal(?:\s|$)([\s\S]*)")
async def handler(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("managers").add_command(
    "ls_local", None, "Gives the list of downloaded medias in your kiarabot server."
).add_command(
    "ls_root", None, "Gives the list of all files in root directory of Kiarabot repo."
).add_command(
    "ls_saved", None, "Gives the list of all files in Saved directory of your kiarabot server"
).add_command(
