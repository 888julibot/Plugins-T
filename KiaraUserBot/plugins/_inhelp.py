...
def button(page, modules):
    ...
    for pairs in pairs[page]:
        buttons.append(
            [
                Button.inline(f"{kiara_emoji} {pair} {kiara_emoji}", data=f"Information[{page}]({pair})")
                for pair in pairs
            ]
        )
    ...
    buttons.append(
        [
            Button.inline(f"⤟ Back {kiara_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"),
            Button.inline(f"• ❌ •", data="close"),
            Button.inline(f"{kiara_emoji} Next ⤠", data=f"page({0 if page == (max_pages - 1) else (page + 1)})"),
        ]
    )
...
if Config.BOT_USERNAME and tbot:
    ...
    async def inline_handler(event):
        ForGo10God, kiara_USER, kiara_mention = await client_id(event, event.query.user_id)
        builder = event.builder
        result = None
        query = event.text
        auth = await clients_list()
        if event.query.user_id in auth and query == "kiarabot_help":
            ...
            if help_pic == "DISABLE":
                ...
                result = builder.document(
                    help_pic,
                    text=help_msg,
                    title="KiaraBot Alive",
                    buttons=veriler[1],
                    link_preview=False,
                )
        elif event.query.user_id in auth and query == "alive":
            uptime = await get_time((time.time() - StartTime))
            alive_msg = gvarstat("ALIVE_MSG") or "»»» <b>ɥʂɛʀɮօt ιѕ σиℓιиє</b> «««"
            alive_name = gvarstat("ALIVE_NAME") or KIARA_USER
            ki_ara = alive_txt.format(
                alive_msg, telethon_version, kiarabot_version, uptime, abuse_m, is_sudo
            )
            alv_btn = [
                [
                    Button.url(f"{alive_name}", f"tg://openmessage?user_id={Kiara_X_Assistant}")
                ],
                [
            ...
            if a:
                ...
                PIC = "https://te.legra.ph/file/27a28dd49b5d2ef62e5ac.mp4"
            ...
            KIARA_FIRST = f"🔥 𝙐𝙨𝙚𝙧𝘽𝙤𝙩 𝙋𝙈 𝙎𝙚𝙘𝙪𝙧𝙞𝙩𝙮 🔥\n\nHello!! This is an automated message on behalf of {kiara_mention}."
            if CSTM_PMP:
                KIARA_FIRST += f"\n\n{CSTM_PMP}"
            ...
            if a and a == "DISABLE":
                ...
                PIC = "https://te.legra.ph/file/413f5e28fbc312981f6a2.jpg"
            if PIC and PIC.endswith((".jpg", ".png")):
                result = builder.photo(
                    file=PIC,
                    text=KIARA_FIRST,
            ...
            result = builder.article(
                "@Kiara_X_Upadates",
                text="""**Hey! This is [Ừʂɛɤẞø†](https://t.me/Kiara_X_Upadates) \nYou can know more about me from the links given below 👇**""",
                buttons=[
                    [
                        Button.url("• Updates •", "https://t.me/Kiara_X_Upadates"),
                        Button.url("• Chat •", "https://t.me/Kiara_x_Support"),
                    ],
                    [
                        Button.url("• Repo •", "https://github.com/zetrodeploy/KiaraUserBot"),
                    ],
    ...
    async def on_pm_click(event):
        ...
        if event.query.user_id in auth:
            ...
            reply_popup = "🔰 This is Ừʂɛɤẞø† PM Security to keep away unwanted retards from spamming PM !!"
    ...
    async def reopn(event):
        _, _, kiara_mention = await client_id(event, event.query.user_id)
        auth = await clients_list()
        if event.query.user_id in auth:
            ...
            if a:
                ...
                help_pic = "https://te.legra.ph/file/413f5e28fbc312981f6a2.jpg"

            if help_pic == "DISABLE":
                await event.edit(
                    text=f"🔰 **{kiara_mention}**\n\n📜 __Plugins:__ `{len(CMD_HELP)}` \n🗂️ __Commands:__ `{len(apn)}`\n🗒️ __Page:__ 1/{veriler[0]}",
                    buttons=simp[1],
                    link_preview=False,
                    file=None,
                )
            ...
            await event.answer("Hello! This help menu is not for you, you can make yourself a KiaraBot and use your bot. Go to @Kiara_X_Upadates for more info.", cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if event.query.user_id in auth:
            veriler = Button.inline(
                f"{kiara_emoji} Re-Open Menu {kiara_emoji}", data="reopen"
            )
            await event.edit(
                f"**🎭 Closed KiaraBot's help menu**\n\n**Bot Of:**  {kiara_mention}\n\n        [©️ Ừʂɛɤẞø† ™️]({chnl_link})",
                buttons=veriler,
                link_preview=False,
            )
        else:
    ...
    async def send(event):
        ...
        the_plugin_file = "./KiaraUserBot/plugins/{}.py".format(plugin.lower())
        butt = Button.inline(f"{kiara_emoji} Main Menu {kiara_emoji}", data="reopen")
    ...
    async def page(event):
        ...
        if event.query.user_id in auth:
            ...
            return await event.answer(
                "Hello! This help menu is not for you, you can make yourself a KiaraBot and use your bot. Go to @Kiara_X_Upadates for more info.",
                cache_time=0,
                alert=True,
            )
    ...
    async def Information(event):
        ...
        buttons.append([Button.inline(f"{kiara_emoji} Main Menu {kiara_emoji}", data=f"page({page})")])
        if event.query.user_id in auth:
            await event.edit(
    ...
    async def commands(event):
        ...
        if event.query.user_id in auth:
            await event.edit(
                ...
                buttons=[
                    Button.inline(
                        f"{kiara_emoji} Return {kiara_emoji}",
