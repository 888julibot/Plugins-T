...
@kiara_cmd(pattern="inviteall(?:\s|$)([\s\S]*)")
async def get_users(event):
    hel_ = event.text[11:]
    kiara = await eor(event, f"__Inviting members from__ {hel_}")
    kraken = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await eod(kiara, "Nice try you fool!")
    s = 0
    f = 0
    error = "None"
    await kiara.edit("**INVITING USERS !!**")
