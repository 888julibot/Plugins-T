...
@kiara_cmd(pattern="autoname$")
async def _(event):
    kiara = await eor(event, "`Starting AutoName Please Wait`")
    _id, kiara_USER, _ment = await client_id(event)
    await kiara.edit(f"Auto Name has been started my Master")
...
@kiara_cmd(pattern="autobio$")
async def _(event):
    kiara = await eor(event, "Starting AutoBio...")
    await kiara.edit("AutoBio Activated...")
