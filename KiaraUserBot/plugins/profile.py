...
async def _(event):
    ...
    if photo:
        ...
        try:
            ...
            await eor(kiara, "**Changed profile to OffLine.**")
    try:
    ...
    try:
        ...
        await eor(kiara, result)
    except Exception as e:
        await parse_error(kiara, e)


@kiara_cmd(pattern="online$")
async def _(event):
    ForGo10God, kiara_USER, _ = await client_id(event)
    user = await event.client.get_entity(ForGo10God)
    if kiara_USER.startswith("[ • OFFLINE • ]"):
        kiara = await eor(event, "**Changing Profile to Online...**")
    else:
    ...
    if photo:
        ...
        try:
            ...
            await eor(kiara, "**Changed profile to Online.**")
    try:
        os.remove("donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
@kiara_cmd(pattern="ppic$")
async def _(event):
    reply = await event.get_reply_message()
    kiara = await eor(event, "Changing profile picture ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            ...
            try:
                ...
                await eod(kiara, "**📍 Profile picture changed successfully.**")
    try:
        os.remove(photo)
    except Exception as e:
        LOGS.warn(str(e))
...
@kiara_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await kiara.edit(result, parse_mode="HTML")


@kiara_cmd(pattern="delpfp(?:\s|$)([\s\S]*)")
...
@kiara_cmd(pattern="myusernames$")
async def _(event):
    _, _, kiara_mention = await client_id(event)
    result = await event.client(GetAdminedPublicChannelsRequest())
    output_str = f"**Usernames reserved by:** {kiara_mention}\n\n"
    for channel_obj in result.chats:
        output_str += f"• {channel_obj.title} ~ @{channel_obj.username} \n"
    await eor(event, output_str)


@kiara_cmd(pattern="stats$")
async def stats(event):
    kiara = await eor(event, "`Collecting stats...`")
    ...
    ForGo10God, kiara_USER, kiara_mention = await client_id(event, is_html=True)
    response = f"<b><i><u>♛ Stats for {kiara_mention} ♛</b></i></u>\n\n"
    ...
    await kiara.edit(response, parse_mode="HTML", link_preview=False)
