...
@kiara_cmd(pattern="wallpaper(?:\s|$)([\s\S]*)")
async def wallpaper(event):
    ...
    if not _wall:
        return await parse_error(kiara, "No wallpapers found.")
    x = 0
    wall_list = []
    for i in _wall:
        bg = requests.get(i)
