...
@kiara_cmd(pattern="font(?:\s|$)([\s\S]*)")
async def font(event):
    kiara = await eor(event, "Changing font...")
    flag = event.text[6:]
    rply = await event.get_reply_message()
    if not rply:
        return await eod(kiara, "Nothing given to change!")
    old = rply.text
    normie = normal_str.split(" ")
    prev = " ".join(old).upper()
    if not flag.isnumeric():
        return await eod(kiara, "Give font numbers only!")
    match int(flag):
        case 1:
            to_ = one.split(" ")
            for normal in prev:
        ...
        case 24:
            ...
            await kiara.edit(prev)
        case _:
            await eod(kiara, "Unsupported font!")
