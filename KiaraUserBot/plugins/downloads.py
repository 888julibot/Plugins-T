...
@kiara_cmd(pattern="download(?:\s|$)([\s\S]*)")
async def _(event):
    kiara = await eor(event, "Downloader in action ...")
    ...
    if event.reply_to_msg_id:
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff % 10.00) == 0 and current_message != display_message:
                    await kiara.edit(current_message)
        ...
        if downloader.isSuccessful():
            ...
            await parse_error(kiara, f"__Incorrect URL:__ {input_str}", False)
    else:
        await eod(kiara, f"**WRONG SYNTAX:** \n\n`{hl}download <reply or url>`")


@kiara_cmd(pattern="getc(?:\s|$)([\s\S]*)")
async def get_media(event):
    ...
    kiara = await eor(event, f"Downloading medias from {channel_username} channel.")
    ...
    await kiara.edit("Downloaded " + output + " files.")


@kiara_cmd(pattern="geta(?:\s|$)([\s\S]*)")
async def get_media(event):
    dir = "./channel_dl/"
    if not os.path.isdir(dir):
        os.makedirs(dir)
    ...
    if not len(lists) >= 2:
        return await eod(event, "Invalid command.")
    kiara = await eor(event, f"Downloading All Media From {lists[1]} Channel.")
...
CmdHelp("downloads").add_command(
    ...
    "getc", "<limit> <channel username>", "Will download latest given number of media from channel into your bot server", "getc 10 @Kiara_X_Upadates"
).add_info(
