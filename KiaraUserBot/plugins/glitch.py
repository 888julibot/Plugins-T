...
# 
# Ported from Pyrogram to Telethon by @ForGo10God for †hê Ừʂɛɤẞø†
# 
# USE WITH CREDITS !!
##################################
...
@kiara_cmd(pattern="glitch(?:\s|$)([\s\S]*)")
async def glitch_(event):
    Glitched = Config.TMP_DOWNLOAD_DIRECTORY + "glitch.gif"
    kiara = await eor(event, "`Trying to glitch this ...`")
    replied = await event.get_reply_message()
    lists = event.text.split(" ", 2)
    if not (replied and (replied.photo or replied.sticker or replied.video or replied.gif)):
        return await parse_error(kiara, "Invalid media format.")
    if len(lists) >= 2:
        if not lists[1].isdigit():
            return await parse_error(kiara, "Input type is not digits.")
        input_ = int(lists[1])
        if not 0 < input_ < 9:
            return await parse_error(kiara, "Input range beyond 1 - 8")
        args = input_
    else:
        args = 2
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    await kiara.edit("`Glitchingggggg`")
    ...
    if dls.endswith(".tgs"):
        ...
        if not os.path.lexists(file_1):
            await parse_error(kiara, "Sticker not found.")
