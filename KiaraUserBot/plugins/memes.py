...
@kiara_cmd(pattern="thug$")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "Reply to a image...")
    kiara = await eor(event, "`Converting To thug Image..`")
    await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    imagePath = img
    maskPath = "./KiaraConfig/resources/pics/mask.png"
    cascPath = "./KiaraConfig/resources/xmls/haarcascade_frontalface_default.xml"
...
@kiara_cmd(pattern="trigger$")
async def _(event):
    kiara = await eor(event, "`Trigggggggggerrr`")
...
@kiara_cmd(pattern="geyy$")
async def _(event):
    kiara = await eor(event, "`Geyyyy`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://te.legra.ph{url_s[0]}"
...
@kiara_cmd(pattern="pix$")
async def _(event):
    kiara = await eor(event, "`Pixing This Image.`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://te.legra.ph{url_s[0]}"
...
@kiara_cmd(pattern="ytc(?:\s|$)([\s\S]*)")
async def _(event):
    kiara = await eor(event, "`Making a youthuub comment...`")
    owo = await event.get_reply_message()
...
@kiara_cmd(pattern="audiomeme(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if meme:
        await event.delete()
        kiara = await meme[0].click(Config.LOGGER_ID)
        if kiara:
            await event.client.send_file(
                event.chat_id,
                kiara,
