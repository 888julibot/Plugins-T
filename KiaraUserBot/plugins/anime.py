...
@kiara_cmd(pattern="anime(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        return await eor(event, "Please give anime name to search on Anilist.")
    kiara = await eor(event, f"__Searching for__ `{query}` __on Anilist.__")
    qdb = rand_key()
    ANIME_DB[qdb] = query
    result = await get_anilist(qdb, 1)
    if len(result) == 1:
        return await kiara.edit(result[0])
    ...
    try:
        ...
        await kiara.edit(msg)
    if os.path.exists(pic):
        os.remove(pic)


@kiara_cmd(pattern="manga(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        await eor(event, "Please give manga name to search..")
    kiara = await eor(event, f"__Searching for__ `{query}` ...")
    qdb = rand_key()
    MANGA_DB[qdb] = query
    ...
    try:
        ...
        await kiara.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)


@kiara_cmd(pattern="character(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[11:]
    if query == "":
    ...
    try:
        ...
        await kiara.delete(cap_text)
    if os.path.exists(img):
        os.remove(img)


@kiara_cmd(pattern="fillers(?:\s|$)([\s\S]*)")
async def canon(event):
    kiara = event.text[9:]
    if kiara == "":
        return await eor(event, "`Give anime name to search filler episodes.`")
    nub = await eor(event, f"Searching Filler Episodes For `{kiara}`")
    hel_ = search_filler(kiara)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{kiara}`")
    ...
    if len(list_) == 1:
        ...
        return
    kiarabot = f"**📃 Filler Episode Lists :** \n\n"
    for i in list_:
        ...
        kiarabot += f"• [{i}]({paste})\n"
    await nub.edit(kiarabot)


@kiara_cmd(pattern="airing(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[8:]
    kiara = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(kiara, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
    ...
    try:
        ...
        await kiara.edit(out)
    if os.path.exists(coverImg):
        os.remove(coverImg)


@kiara_cmd(pattern="aniuser(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[9:]
    kiara = await eor(event, "Searching user's Anilist Stats...")
    if query == "":
        return await kiara.edit("No user found. Give anilist username.")
    qry = {"search": query}
    result = await get_user(qry)
    if len(result) == 1:
        return await eod(kiara, result[0])
    pic, msg = result
    try:
        await event.client.send_file(
            event.chat_id,
...
CmdHelp("anime").add_command(
    ...
    "aniuser", "<anilist username>", "Searches for the Anilist Stats of the given user.", "aniuser meizkiarabot"
).add_command(
