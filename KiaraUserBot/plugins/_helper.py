...
msg = f"""
**⚡ ʟɛɢɛռɖaʀʏ ᴀғ ɥʂɛʀɮօt ⚡**
  •        [📑 Repo 📑](https://github.com/zetrodeploy/KiaraUserBot)
  •  ©️ {kiara_channel} ™
"""


@kiara_cmd(pattern="repo$")
async def repo(event):
    Kiara_X_Assistant, _, _ = await client_id(event)
    try:
        kiara = await event.client.inline_query(Config.BOT_USERNAME, "repo")
        await kiara[0].click(event.chat_id)
        if event.sender_id == Kiara_X_Assistant:
            await event.delete()
    except (noin, dedbot):
        await eor(event, msg)


@kiara_cmd(pattern="help$")
async def _(event):
    if Config.BOT_USERNAME:
        try:
            ...
            kiara = await eor(event, "**Inline Mode is disabled.** \n__Turning it on, please wait for a minute...__")
            async with bot.conversation("@BotFather") as conv:
                try:
                    ...
                    return await parse_error(kiara, "__Unblock__ @Botfather __first.__", False)
                await eod(kiara, f"**Turned On Inline Mode Successfully.** \n\nDo `{hl}help` again to get the help menu.")
...
@kiara_cmd(pattern="plinfo(?:\s|$)([\s\S]*)")
async def kiarabott(event):
...
@kiara_cmd(pattern="cmdinfo(?:\s|$)([\s\S]*)")
