...
@kiara_cmd(pattern="reverse(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    link = alls["href"]
    text = alls.text
    await kiara.edit(f"**Possible Results:** [{text}](google.com{link})")
...
async def gps(event):
    ...
    kiara = await eor(event, "Finding😁")
    geolocator = Nominatim(user_agent="kiarabot")
...
async def _(event):
    ...
    try:
        ...
        await kiara.edit("Starting Google Chrome BIN")
        driver = webdriver.Chrome(chrome_options=chrome_options)
        input_str = event.pattern_match.group(1)
        driver.get(input_str)
        await kiara.edit("Calculating Page Dimensions")
        ...
        await kiara.edit("Painting web-page")
        driver.set_window_size(width + 100, height + 100)
        im_png = driver.get_screenshot_as_png()
        driver.close()
        await kiara.edit("Stopping Google Chrome BIN")
        ...
        with io.BytesIO(im_png) as out_file:
            out_file.name = "kiara_Capture.PNG"
            await event.client.send_file(
                event.chat_id,
                out_file,
                caption=caption,
        ...
        await parse_error(kiara, traceback.format_exc())


@kiara_cmd(pattern="cricket$")
async def _(event):
...
CmdHelp("google").add_command(
    "google", "<query>", "Does a google search for the query provided", "google kiarabot"
).add_command(
    ...
    "webshot", "<link>", f"Gives out the web screenshot of given link via Google Crome Bin in .png format", "webshot https://github.com/kiarabot-op/kiarabot"
).add_command(
