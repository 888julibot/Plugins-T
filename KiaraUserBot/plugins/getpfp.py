...
@kiara_cmd(pattern="getpic(?:\s|$)([\s\S]*)")
async def _(event):
    kiara = await eor(event, "Getting profile photo..")
    replied_user, error_i_a = await get_full_user(event)
    if replied_user is None:
        await kiara.edit(str(error_i_a))
        return False
...
@kiara_cmd(pattern="poto(?:\s|$)([\s\S]*)")
async def potocmd(event):
    uid = "".join(event.raw_text.split(maxsplit=1)[1:])
    user = await event.get_reply_message()
    chat = event.input_chat
    kiara = await eor(event, "Getting profile pictures of this user...")
    if user:
    ...
    if uid.strip() == "":
        ...
        if int(uid) <= (len(photos)):
            ...
            await eod(kiara, "No photo found of this NIBBA. Now u Die!")
            return
    elif uid.strip() == "all":
        if len(photos) > 0:
            ...
            try:
                ...
                await eod(kiara, "**This user has no photos!**")
                return
    else:
        try:
            uid = int(uid)
            if uid <= 0:
                await parse_error(kiara, "Invalid Numbers.")
                return
        except BaseException:
            await parse_error(kiara, "Unexpected Error!")
            return
        if int(uid) <= (len(photos)):
            send_photos = await event.client.download_media(photos[uid - 1])
            await event.client.send_file(event.chat_id, send_photos)
