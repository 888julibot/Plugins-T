...
async def download(event):
    ...
    kiara = await eor(event, "IG downloader in action...")

    if flag.lower() == "-htag":
        if link and link.startswith("#"):
            await kiara.edit(f"IG downloader in action... \n\nUploading top {post} posts of `{link}`")
            try:
                    ...
                    for single in items_list:
                        try:
                            await event.client.send_file(
                                event.chat_id, 
                                single,
                                caption=f"📥 InstaGram Post Downloaded By :- {kiara_mention}",
                            )
                            count += 1
                        except Exception as e:
                            LOGS.info(str(e))
                        os.remove(single)
                await kiara.edit(f"**Downloaded top posts of** `{link}` \n\n__Total:__ `{count} posts.`")
            except Exception as e:
                return await parse_error(kiara, e)
        else:
            return await eod(kiara, f"**SYNTAX EXAMPLE:** \n\n`{hl}igdl -htag #amvs`\n\nThis will give top 10 IG posts of hashtag `#amvs`.")
    elif link:
        ...
        try:
            ...
            return await parse_error(kiara, e)
        await kiara.edit("**Downloaded post.** __Uploading now ...__")
        items_list = [os.path.join('./insta/dl', file) for file in os.listdir('./insta/dl')]
        count = 0
        if items_list != []:
            for single in items_list:
            ...
            await kiara.edit(f"**Downloaded Instagram Post!** \n\n__Total:__ `{count} posts.`")
        else:
            await parse_error(kiara, "Unable to upload video! Check LOGS and try again!")
    else:
        await parse_error(kiara, "Give proper command.")


@kiara_cmd(pattern="igup(?:\s|$)([\s\S]*)")
async def upload(event):
    flag, url = await get_flag(event)
    kiara = await eor(event, "IG uploader in action...")
    reply = await event.get_reply_message()
    caption = f"{reply.message} \n\n" if reply.message else ""
    caption += "• #UploadedByKiaraBot"
    kiara_MEDIA = media_type(reply)
    ...
    if not reply:
        return await eod(kiara, "Reply to a media to upload on instagram.")
    if not reply.media:
        return await eod(kiara, "Reply to a media to upload on instagram.")

    if flag.lower() == "-reel":
        if kiara_MEDIA not in ["Gif", "Video"]:
            return await eod(kiara, "A reel can only be GIF or Video!")
        file = await event.client.download_media(reply)
        await kiara.edit("**Downloaded!** \n\nNow uploading reel to instagram...")
        try:
            video = IG.clip_upload(path=file, caption=caption)
        except Exception as e:
            os.remove(file)
            return await parse_error(kiara, e)
        await kiara.edit(f"**Uploaded Reel to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)
    
    elif flag.lower() == "-tv":
        if kiara_MEDIA not in ["Gif", "Video"]:
            return await eod(kiara, "An IGTV can only be GIF or Video!")
        file = await event.client.download_media(reply)
        ...
        try:
            ...
            os.remove(file)
            return await parse_error(kiara, e)
        await kiara.edit(f"**Uploaded IGTV to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)
    
    elif flag.lower() == "-vid":
        if kiara_MEDIA not in ["Gif", "Video"]:
            return await eod(kiara, "A video post can only be GIF or Video!")
        file = await event.client.download_media(reply)
        ...
        try:
            ...
            os.remove(file)
            return await parse_error(kiara, e)
        await kiara.edit(f"**Uploaded Video to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)

    elif flag.lower() == "-pic":
        if kiara_MEDIA != "Photo":
            return await eod(kiara, "A picture post can only be a Photo!")
        file = await event.client.download_media(reply)
        ...
        try:
            ...
            os.remove(file)
            return await parse_error(kiara, e)
        await kiara.edit(f"**Uploaded Photo to Instagram!** \n\n[See Post From Here](https://instagram.com/p/{video.code})", link_preview=False)
        os.remove(file)

    elif flag.lower() == "-story":
        if kiara_MEDIA in ["Gif", "Video"]:
            file = await event.client.download_media(reply)
            ...
            return await parse_error(kiara, "Invalid media format. Only Videos, Pictures, GIF are supported to upload story.")

    else:
        await eod(kiara, f"Give proper flag. Check `{hl}plinfo instagram` for details.")
    

@kiara_cmd(pattern="iguser(?:\s|$)([\s\S]*)")
async def userinfo(event):
    uname = event.text.split(" ", 2)[1]
    username = uname.replace("@", "") if "@" in uname else uname
    kiara = await eor(event, f"Searching `{username}` on Instagram...")
    info_str = """
...
"""
    ...
    if IG:
        ...
        await parse_error(kiara, "`INSTAGRAM_SESSION` __not configured or Expired !__", False)
