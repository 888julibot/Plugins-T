...
@kiara_cmd(pattern="paste(?:\s|$)([\s\S]*)")
async def _(event):
    kiara = await eor(event, "`Pasting ....`")
    lists = event.text.split(" ", 1)
    reply = await event.get_reply_message()
    if not reply and not len(lists) == 2:
        return await parse_error(kiara, "Nothing given to paste.")
    if len(lists) == 2:
    ...
    if text_to_print == "" and reply.media:
        ...
        if mediatype == "Document":
            ...
            return await parse_error(kiara, "Reply to a document only.")
    if text_to_print == "":
        if reply.text:
            text_to_print = reply.raw_text
        else:
            return await parse_error(kiara, "Nothing to paste.")
    if extension and extension.startswith("."):
        extension = extension[1:]
    try:
        response = await pasty(event, text_to_print, extension)
        if "error" in response:
            return await parse_error(kiara, "Error while pasting text.")
        result = f"<b><i>📍 Pasted To</i> <a href={response['url']}>Here</a></b>"
        if response["raw"] != "":
            result += f"\n<b><i>📃 Raw link:</i> <a href={response['raw']}>Raw</a></b>"
        await kiara.edit(result, link_preview=False, parse_mode="html")
    except Exception as e:
        await parse_error(kiara, e)


@kiara_cmd(pattern="neko(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    kiara = await eor(event, "Pasting to nekobin...")
    if len(lists) == 2:
        message = lists[1]
    elif reply:
        if reply.media:
    ...
    await eor(kiara, reply_text, link_preview=False)
