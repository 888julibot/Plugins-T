...
async def _(event):
    ...
    if type_ == 'Sticker':
        kiara_ = await event.client.download_media(rply.media)
        gifs = await tgs_to_gif(kiara_, True)
        unsave =  await event.client.send_file(event.chat_id, file=gifs, force_document=False)
        await unsave_gif(event, unsave)
        await kiara.delete()
        os.remove(kiara_)
        os.remove("kiarabot.gif")
    else:
        await parse_error(kiara, "Only animated stickers are supported.")


@kiara_cmd(pattern="stoi$")
async def _(kiara):
    reply_to_id = kiara.message.id
    if kiara.reply_to_msg_id:
        reply_to_id = kiara.reply_to_msg_id
    event = await eor(kiara, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await kiara.client.download_media(
...
async def _(event):
    ...
    try:
        ...
        await kiara.edit(f"__Downloaded:__ `{downloaded_file_name}` \n__Time taken:__ `{ms} seconds`")
        ...
        if input_str == "voice":
            ...
            await parse_error(kiara, "Not supported")
            os.remove(downloaded_file_name)
            return
        ...
        if os.path.exists(new_required_file_name):
            ...
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, kiara, c_time, "Uploading ...")
                ),
            )
