...
@kiara_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
    kiaraevent = await eor(event, "...")
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    ...
    try:
        ...
        await kiaraevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await kiaraevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await kiaraevent.delete()


@kiara_cmd(pattern="makeqr(?: |$)([\s\S]*)")
...
@kiara_cmd(pattern="calendar(?:\s|$)([\s\S]*)")
async def _(event):
...
@kiara_cmd(pattern="currency(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if lists[1].strip().isdigit():
        ...
        return await parse_error(event, f"__Give proper command:__ \n__Ex:__`{hl}currency 10 usd inr`", False)
    kiara = await eor(event, f"**Converting currency:** \n__From:__ `{base}` \n__To:__ `{into}` \n__Amount:__ `{amnt}`")
    ...
    try:
        if data['result'] == 'success':
            ...
            await kiara.edit(f"**Currency Converted !!** \n\n{output}")
        else:
            await eod(kiara, "Something went wrong. Try again later!")
    except Exception as e:
        await parse_error(kiara, e)


@kiara_cmd(pattern="currencies$")
async def currencylist(event):
    if not Config.CURRENCY_API:
        return await parse_error(event, "`CURRENCY_API` __not configured.__", False)
    kiara = await eor(event, "Fetching supportred currencies lists ...")
    ...
    try:
        ...
        link = await telegraph_paste("Currency List For KiaraBot", output)
        await kiara.edit(
...
@kiara_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    kiaraevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await kiaraevent.edit(output_str, link_preview=True)
    else:
        await eod(kiaraevent, "xkcd n.{} not found!".format(xkcd_id))


@kiara_cmd(pattern="dns(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
@kiara_cmd(pattern="unshort(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
...
CmdHelp("tools").add_command(
    ...
    "currency", "<amount> <from> <to>", "Currency converter for KiaraBot", ".currency 10 usd inr"
).add_command(
