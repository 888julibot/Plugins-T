...
@kiara_cmd(pattern="love$")
async def love(e):
    txt = choice(LOVESTR)
    await eor(e, txt)


@kiara_cmd(pattern="dhoka$")
async def katgya(e):
    txt = choice(DHOKA)
    await eor(e, txt)


@kiara_cmd(pattern="metoo$")
async def metoo(e):
    txt = choice(METOOSTR)
    await eor(e, txt)


@kiara_cmd(pattern="gdnoon$")
async def noon(e):
    txt = choice(GDNOON)
    await eor(e, txt)


@kiara_cmd(pattern="chase$")
async def police(e):
    txt = choice(CHASE_STR)
    await eor(e, txt)


@kiara_cmd(pattern="congo$")
async def Sahih(e):
    txt = choice(CONGRATULATION)
    await eor(e, txt)


@kiara_cmd(pattern="qhi$")
async def hoi(e):
    txt = choice(HELLOSTR)
    await eor(e, txt)


@kiara_cmd(pattern="gdbye$")
async def bhago(e):
    txt = choice(BYESTR)
    await eor(e, txt)


@kiara_cmd(pattern="gdnyt$")
async def night(e):
    txt = choice(GDNIGHT)
    await eor(e, txt)


@kiara_cmd(pattern="gdmng$")
async def morning(e):
    txt = choice(GDMORNING)
    await eor(e, txt)


@kiara_cmd(pattern="quote(?:\s|$)([\s\S]*)")
async def quote_search(event):
    ...
    if response is not None:
        await kiara.edit(f"`{response['text']}`")
    else:
        await parse_error(kiara, "No quote found! Try again later.")
